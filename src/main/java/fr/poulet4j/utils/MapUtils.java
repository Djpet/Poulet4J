...
public class MapUtils {
	 ...
	 */
	public static Path getPath(final Cell source, final Cell destination, final GameMap gameMap) {
		...
		while (!queue.isEmpty() && dest == null) {
			...
			CellExtended tmp = calcul(queue, map, ce, ce.cell.topCell, destination);
			if (tmp != null) {
				dest = tmp;
				break;
			}
			...
			tmp = calcul(queue, map, ce, ce.cell.leftCell, destination);
			if (tmp != null) {
				dest = tmp;
				break;
			}
	 ...
	 */
	private static CellExtended calcul(final Queue<CellExtended> queue, final CellExtended[][] map, final CellExtended current, final Cell next,
